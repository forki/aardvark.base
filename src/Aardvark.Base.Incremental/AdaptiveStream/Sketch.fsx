...
module SketchObs =
    ...
    type obs<'a> = IObservable<'a>
//
//    module Observable =
//        let inline collect (f : 'a -> obs<'b>) (o : obs<'a>) =
//            o.SelectMany f
//
//        let inline map (f : 'a -> 'b) (o : obs<'a>) =
//            o.Select(f)
//
//        let inline single (v : 'a) =
//            Observable.Never<'a>().StartWith([|v|]) 
//
//        let inline ofSeq (v : seq<'a>) =
//            Observable.Never<'a>().StartWith(v) 
//
//        let inline ofList (v : list<'a>) =
//            Observable.Never<'a>().StartWith(v) 
//
//        let inline ofArray (v : 'a[]) =
//            Observable.Never<'a>().StartWith(v) 
//
//        let partition (cond : obs<bool>) (values : obs<'a>) =
//            Observable.W
    ...
    type ObservableBuilder() =
        ...
        member x.Yield(v) = Observable.Never<'a>().StartWith([|v|]) 

        member x.Combine(l : IObservable<'a>, r : IObservable<'a>) =
            Observable.Concat(l, r)

        member x.Delay(f : unit -> IObservable<'a>) = Observable.Defer(f)

        member x.While(guard : unit -> bool, body : IObservable<'a>) =
            Observable.While(Func<bool>(guard), body)

        member x.While(guard : unit -> IObservable<bool>, body : IObservable<'a>) =
            let guard = guard()
            Observable.Create(fun obs ->
                let mutable active = false
                let gs = guard.Subscribe(fun v -> active <- v)
    ...
    let test (active : IObservable<bool>) (pos : IObservable<V2i>) =
        obs {
            while true do
                for e in a do
                    yield 2 * e
    ...
    let run() =
        ...
        test.Subscribe(fun v -> printfn "test: %A" v) |> ignore
